...
use Getopt::Long;
my $version = '1.0.0';
open STDERR, ">>./cFLSeq.log";
### Parameters can be designated by user
my ( $circ, $genome, $output, $help, $gtf, $thread, $read1, $read2, $contig );
...
";
} elsif ( !defined($circ) or !defined($genome) or !defined($output) or !defined($thread) or !defined($read1) or !defined($read2) or !defined($contig)) {
        ...
        if ($frag_num % 2 == 0) {
          ...
          for $i (1 .. ($frag_num/2 - 1)) {
            ...
            if (($frag_count{$circ_id}{$frag_del_new[$i]} - $frag_count{$circ_id}{$frag_del_new[$j]}) > 2) {
              ...
              if ($frag_count{$circ_id}{$frag_del_new[$i]} < $frag_count{$circ_id}{$frag_del_new[$i-1]}) {
                if ($frag_count{$circ_id}{$frag_del_new[$i]} < $frag_count{$circ_id}{$frag_del_new[$i+1]}) {
                  #print $circ_id."\t".$frag_del_new[$i]."\n";
